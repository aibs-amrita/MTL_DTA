...
def main():
    ...
    dirpath   = os.path.join(data_path,"models/davis", timestamp)
...
class pl_class(DeepDTA):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #kiba
        y_pred,_,_    = self.forward(kiba)
        (_,_,y_true)  = kiba
        kiba_loss     = self.loss_function(y_pred.view(-1), y_true)
        kiba_ci       = self.concordance_index(y_true.detach().cpu().numpy(), y_pred.detach().cpu().numpy())

        # metz
        _,y_pred,_    = self.forward(metz)
        (_,_,y_true)  = metz
        ...
        avg_loss = (kiba_loss + metz_loss + davis_loss)/3
        avg_ci   = (kiba_ci + metz_ci + davis_ci)/3
    ...
    def validation_step(self, batch, batch_idx):
        ...
        kiba_loss     = self.loss_function(y_pred.view(-1), y_true)
        kiba_ci       = self.concordance_index(y_true.detach().cpu().numpy(), y_pred.detach().cpu().numpy())
        kiba_r2       = self.r2_score(y_true.detach().cpu().numpy(), y_pred.detach().cpu().numpy())
...
class data_class(pl.LightningModule):
    ...
    def __init__(self, data_path, batch_size):
        ...
        self.train_kiba  = torch.load(os.path.join(self.data_path, "kiba/processed/train_set.pth"))
        ...
        self.test_kiba   = torch.load(os.path.join(self.data_path, "kiba/processed/test_set.pth"))
    ...
    def train_dataloader(self):
        kiba_train_loader  = DataLoader(self.train_kiba, batch_size = self.batch_size)
        ...
        loaders = {"kiba": kiba_train_loader, "metz": metz_train_loader, "davis": davis_train_loader}
    ...
    def test_dataloader(self):
        kiba_test_loader  = DataLoader(self.test_kiba, batch_size = self.batch_size)
        ...
        loaders = {"kiba": kiba_test_loader, "metz": metz_test_loader, "davis": davis_test_loader}
